...
`
    ...
    try {
      ...
      collector.on('end', collected => {
        const maxCount = Math.max.apply(Math, collected.map(o => o.count));
...
const collectTimes = (channel, game, date, notifyTime, minPlayers, maxPlayers, message) => {
  const collector = message.channel.createMessageCollector(m => m.content.length >= 5 && m.author.id === message.author.id, { time: timeout });
  collector.on('collect', m => {
    collector.stop();
    message.author.send(`Jatkossa voit halutessasi postata plänin suoraan kanavalle näin:\n!plan "${game}" ${date} ${notifyTime} ${minPlayers} ${maxPlayers} ${m.content}`);
...
const collectMaxPlayers = (channel, game, date, notifyTime, minPlayers, message) => {
  const collector = message.channel.createMessageCollector(m => !isNaN(m.content) && m.author.id === message.author.id, { time: timeout });
  collector.on('collect', m => {
    collector.stop();
    message.author.send(`${date} ${game}, ${minPlayers}-${m.content} pelaajaa. Mitkä kellonajat annetaan vaihtoehdoiksi? Voit antaa useamman ajan välilyönnillä eroteltuna muodossa HH:mm.`);
...
const collectMinPlayers = (channel, game, date, notifyTime, message) => {
  ...
  collector.on('collect', m => {
    collector.stop();
    message.author.send(`${date} ${game} minimissään ${m.content} pelaajalla. Mikä on maksimi pelaajamäärä? Anna 0, jos ei ole maksimia.`);
...
const collectDate = (channel, game, message) => {
  const collector = message.channel.createMessageCollector(m => m.content.length >= 2 && m.author.id === message.author.id, { time: timeout });
  collector.on('collect', m => {
    collector.stop();
...
const collectChannel = (guild, message) => {
  if (guild.channels.cache.filter(c => c.type === "text").size === 1) {
    ...
    const collector = message.channel.createMessageCollector(m => !isNaN(m.content) && parseInt(m.content) >= 1 && parseInt(m.content) <= i-1 && m.author.id === message.author.id, { time: timeout });
    collector.on('collect', m => {
      collector.stop();
...
const collectGuild = (message) => {
  if (message.client.guilds.cache.size === 1) {
    ...
    let i = 1;
    const mutualGuilds = message.client.guilds.cache.filter(guild => guild.members.cache.find(u => u.id === message.author.id)).map(g => g);
...
const planner = {
  ...
  usage: `[pelin nimi] [päivä] [keräysaika tunteina] [min pelaajaa] [max pelaajaa] [...kellonajat (max ${reactOptions.length})]`,
    ...
    publishPlan(message.channel, arguments[0], arguments[1], parseInt(arguments[2]), parseInt(arguments[3]), parseInt(arguments[4]), arguments.slice(5))
	},
};
